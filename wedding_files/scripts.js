...
! function (u) {
                ...
                $("#wish-form").validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        $.ajax({
                            ...
                            //     if (!res.error) {
                            //         // $('#wish-box').append(`<div class="wish-box-item" data-id=$id>
                            //         // <strong>$name</strong>
                            //         // <p>$content</p>
                            //         // </div>`.replace('$id', item._id).replace('$name', item.name).replace('$content', item.content));
                            //     }
                            ...
                            if (res.status < 300) {
                                ...
                                $('#wish-box').prepend(`<div class="wish-box-item" data-id=$id>
                                    <strong>$name</strong>
                                    <p>$content</p>
                                    </div>`.replace('$id', item._id).replace('$name', item.name).replace('$content', item.content).replace(/\n/g, '<br>'));
                                form.reset();
                            }
                        });
                        return false;
    ...
    function loadWish() {
        const wishes = $('#wish-box .wish-box-item');
        ...
        $.get(url, function (res) {
            ...
            console.log('tota page: ', res?.meta?.totalPages);
            if ((res?.meta?.totalPages || 0) > 1) {
    ...
    $(window).on("load", function () {
        ...
        setTimeout(function () {
            ...
            $('#youtube').append(`<iframe width="560" height="315"
            src="https://www.youtube.com/embed/kl4nL6_xbCM?si=MQVnPHKPOowQ4MVp"
    ...
    $(document).on('ready', function () {
        const links = $('#albums a');
        for (const link of links) {
            link.href = link.href.replace('albums', 'large-albums');
        }
    });
